          ...
          if (embed) {
            if (embed.description) {
              if (embed.description.includes('got the card! ')) {
                ...
                const res = await msg.client.ch.query('SELECT * FROM shoob WHERE userid = $1;', [
                  user.id,
                ]);
                let amount = 0;
                if (res && res.rowCount > 0) {
                  amount = res.rows[0].amount + 1;
                  msg.client.ch.query('UPDATE shoob SET amount = $2 WHERE userid = $1;', [
                    user.id,
                    amount,
                  ]);
                } else {
                ...
                const fiftyRole = msg.guild.roles.cache.find(
                  (role) => role.id === '756331367561822258',
                );
                const hundretRole = msg.guild.roles.cache.find(
                  (role) => role.id === '756331587616112660',
                );
                const fivehundretRole = msg.guild.roles.cache.find(
                  (role) => role.id === '756332260805967882',
                );
                const kRole = msg.guild.roles.cache.find(
                  (role) => role.id === '756597164850806896',
                );
