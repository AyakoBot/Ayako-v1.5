...
const setting: CT.MultiSettings = {
  ...
  listEmbed: async (baseObject) => {
    const lan = baseObject.language.slashCommands.settings.settings['blacklist-punishments'];
    const existing = await client.ch
      .query(`SELECT * FROM blacklistpunishments WHERE guildid = $1;`, [
...
const getSettings = (baseObject: CT.MultiSettingsObject) =>
  client.ch
    .query(`SELECT * FROM blacklistpunishments WHERE guildid = $1 AND uniquetimestamp = $2;`, [
      baseObject.interactions[0].guildID,
