...
module.exports = {
  ...
  type: 'auto',
  execute: async (cmd) => {
    cmd.lan = cmd.language.verification;
    ...
    if (res && res.rowCount > 0) {
      ...
      if (r.pendingrole && !cmd.member.roles.cache.has(r.pendingrole)) {
        cmd.client.ch.reply(cmd, {
          content: cmd.lan.alreadyVerified,
          ephemeral: true,
        });
        return;
      }
  ...
  startProcess: async (cmd, answer, logchannel) => {
    if (cmd.m) await cmd.m.removeAttachments();
    else {
      cmd.client.ch.reply(cmd, {
        content: cmd.lan.checkDMs,
        ephemeral: true,
      });
    }
    ...
    if (logchannel) {
      const log = new Builders.UnsafeEmbedBuilder()
        .setDescription(cmd.client.ch.stp(cmd.lan.log.start, { user: cmd.user }))
        .setAuthor({
          name: cmd.user.tag,
    ...
    if (!cmd.m || !cmd.m.id) {
      return cmd.client.ch.reply(cmd, cmd.client.channels.cache.get(r.startchannel), {
        content: cmd.client.ch.stp(cmd.lan.openDMs, {
          user: cmd.user,
    ...
    buttonsCollector.on('collect', (clickButton) => {
      if (clickButton.customId === 'regenerate') {
        ...
        return module.exports.startProcess(cmd, clickButton, logchannel);
      }
      return null;
    });
    ...
    messageCollector.on('collect', async (message) => {
      ...
      if (message.content.toLowerCase() === cmd.language.cancel.toLowerCase()) {
        cmd.m.delete().catch(() => {});
        cmd.client.ch.reply(message, { content: cmd.language.aborted });
        return;
      }
      ...
      if (!captcha) {
        cmd.client.ch.reply(message, { content: `${cmd.language.error} ${cmd.lan.error}` });
        cmd.client.ch.send(
      ...
      if (message.content.toLowerCase() === captcha.toLowerCase()) {
        ...
        module.exports.finished(cmd, logchannel);
        return;
      }
      const ms = await cmd.client.ch.send(cmd.DM, {
        content: cmd.client.ch.stp(cmd.lan.wrongInput, { solution: captcha }),
      });
      ...
      if (reason === 'time') {
        ...
        cmd.m
          .edit({
            embeds: [],
            components: [],
            content: `${cmd.client.ch.stp(cmd.lan.timeError, {
  ...
  finished: async (cmd, logchannel) => {
    if (logchannel) {
      const log = new Builders.UnsafeEmbedBuilder()
        .setDescription(cmd.client.ch.stp(cmd.language.verification?.log?.end, { user: cmd.user }))
        .setAuthor({
          name: cmd.user.tag,
          iconURL: cmd.user.displayAvatarURL({ size: 4096 }),
        })
    ...
    const embed = new Builders.UnsafeEmbedBuilder()
      ...
      .setDescription(cmd.client.ch.stp(cmd.lan.finishDesc, { guild: cmd.guild }))
      .setColor(cmd.client.constants.standard.color);
