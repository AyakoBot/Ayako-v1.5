  ...
  async execute() {
    ...
    if (res && res.rowCount > 0) {
      res.rows.forEach(async (row) => {
        let webhook = await client.fetchWebhook(row.webhook, row.token).catch(() => {});
        if (webhook) client.channelWebhooks.set(row.channel, webhook);
          ...
          if (channel) {
            ...
            webhook = await channel
              .createWebhook(Constants.standard.webhook.name, {
                avatar: Constants.standard.webhook.avatar,
                reason: language.webhookCreate.reason,
              })
              .catch((e) => {
                ch.logger('Create Webhook Error', e);
              });
            if (webhook) {
              client.channelWebhooks.set(channel.id, webhook);
              ch.query(
                `
                            UPDATE webhooks SET webhook = $1, token = $3 WHERE channel = $2; 
                            `,
