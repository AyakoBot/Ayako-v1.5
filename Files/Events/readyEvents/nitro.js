...
module.exports = {
  async execute() {
          ...
          if (role && role.id) {
            role.members.forEach(async (member) => {
              const res = await ch.query(
                `SELECT * FROM nitroboosters WHERE userid = '${member.user.id}' AND guildid = '${guild.id}';`,
              );
              if (res.rowCount == 0) {
                ch.query(
                  `INSERT INTO nitroboosters (guildid, userid, days, stillactive) VALUES ('${guild.id}', '${member.user.id}', '0', 'true');`,
                );
                content += `ADDED to true = ${member.user.id} / ${member.user.username} - ${guild.id} / ${guild.name}\n`;
              }
              if (res.rowCount !== 0) {
                ch.query(
                  `UPDATE nitroboosters SET stillactive = 'true' WHERE userid = '${member.user.id}' AND guildid = '${guild.id}';`,
                );
        ...
        if (res.rows[i].stillactive == true) {
          ...
          if (guild && guild.id) {
            ...
            if (user && user.id) {
              ...
              const resS = await ch.query(
                `SELECT * FROM nitrosettings WHERE guildid = '${guild.id}';`,
              );
              if (resS && resS.rowCount > 0) {
                const { boosterroleid } = resS.rows[0];
                const boosterrole = guild.roles.cache.get(boosterroleid);
                const { nitrologchannelid } = resS.rows[0];
                let NitroLogChannel;
                if (nitrologchannelid !== null) {
                  NitroLogChannel = guild.channels.cache.get(nitrologchannelid);
                }
                const resR = await ch.query(
                  `SELECT * FROM nitroroles WHERE guildid = '${guild.id}';`,
                );
                if (resR.rowCount == 0) {
                  if (member) {
                    if (member.roles.cache.has(boosterrole.id)) {
                      ...
                      Days++;
                      ch.query(
                        `UPDATE nitroboosters SET days = '${Days}' WHERE userid = '${user.id}' AND guildid = '${guild.id}';`,
                      );
                      content += `UPDATED days = ${user.id} / ${user.username} - ${guild.id} / ${guild.name}\n`;
                    } else if (res.rows[i].stillactive == true) {
                      ch.query(
                        `UPDATE nitroboosters SET stillactive = 'false' WHERE userid = '${user.id}' AND guildid = '${guild.id}';`,
                      );
                            ...
                            if (member.roles.cache.has(boosterrole.id)) {
                              if (!member.roles.cache.has(specialrole.id)) {
                                ...
                                const logembed = new Discord.MessageEmbed()
                                  .setDescription(
                                    ch.stp(language.nitro.gotRole, {
                                      user,
                                      role: specialrole,
                                      days: resR.rows[l].days,
                                    }),
                                  )
                                  .setTimestamp()
    ...
    console.log(content);
    Logchannel.send(
      `Finished Nitro Boosting - Check console at ${new Date(Date.now()).toLocaleString()}`,
    ).catch(() => {});
  },
};
