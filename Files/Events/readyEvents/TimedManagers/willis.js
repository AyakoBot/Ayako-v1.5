...
module.exports = async () => {
  ...
  const channel = client.channels.cache.get('979811225212956722');
  const { ch } = client;
  if (channel) {
    const m = await channel.messages.fetch('979934404770144296').catch(() => {});
    if (m && m.id) {
      ...
      needed.forEach((maxparticipants, j) => {
        ...
        // }
        const link = 'https://clcr.me/Raid_WiLLiS';

        const e = new Builders.UnsafeEmbedBuilder()
          .setAuthor({
            name: 'MEGA GIVEAWAY! [You can also click here]',
            iconURL:
          ...
          .setDescription(
            `<:wish:804075599974367242> Any 5 Star Main Banner Character + <:wish:804075599974367242> Their Weapon. \n(Or a $100 Giftcard winner can pick)${
              '' // \n${needed[0]} ${emote[0]}\n${loltext}
            }\n`,
          )
          .addFields({
          ...
          .setImage(
            'https://cdn.discordapp.com/attachments/979811225212956722/979819428164280430/unknown.png',
          )
          .addFields({ name: 'Current Participants:', value: participants });
        if (m.embeds[0].data.fields[2]) {
          if (`${m.embeds[0].data.fields[2].value}` !== `${participants}`) {
            m.client.ch.edit(m, { embeds: [e] });
          }
        } else {
          m.client.ch.edit(m, { embeds: [e], content: '\u200b' });
        }
      });
    }
  }
