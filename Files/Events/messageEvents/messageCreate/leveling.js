    ...
    if (resG && resG.rowCount > 0) {
      ...
      if (user && user.id) {
        if (!cooldown.has(msg.author.id)) {
          ...
          if (resSpam) {
            if (resSpam.rows[0])
              msg.client.ch.query('UPDATE antilevelspam SET message = $1 WHERE userid = $2;', [
                msg.content,
                msg.author.id,
              ]);
            else
              msg.client.ch.query('INSERT INTO antilevelspam (userid, message) VALUES ($2, $1);', [
                msg.content,
                msg.author.id,
              ]);
          } else
          ...
          await msg.client.ch.query('UPDATE levelglobal SET xp = $1 WHERE userid = $2;', [
            XP,
            user.id,
          ]);
          ...
          if (+XP > +neededXP)
            await msg.client.ch.query('UPDATE levelglobal SET level = $1 WHERE userid = $2;', [
              newLevel,
              user.id,
            ]);
        }
      ...
      msg.client.ch.query('INSERT INTO levelglobal(userid, xp, level) VALUES ($1, $2, $3);', [
        msg.author.id,
        1,
        0,
      ]);
    const resS = await msg.client.ch.query('SELECT * FROM levelsettings WHERE guildid = $1;', [
      msg.guild.id,
    ]);
    let settings;
    if (resS && resS.rowCount > 0) [settings] = resS.rows;
    else {
      settings = [];
    ...
    if (settings.disabled === false) {
      ...
      if (!cooldownServer.has(msg.author.id)) {
        ...
        const result = await msg.client.ch.query(
          'SELECT * FROM levelserver WHERE userid = $1 AND guildid = $2;',
          [msg.author.id, msg.guild.id],
        );
        if (result && result.rowCount > 0) {
          if (result.rows[0].blroleid) {
            [...result.rows[0].blroleid.entries()].forEach((id) => {
              const role = msg.guild.roles.cache.get(id);
              if (!role) {
                result.rows[0].blroleid.splice(result.rows[0].blroleid.indexOf(id), 1);
                msg.client.ch.query('UPDATE levelserver SET blroleid = $1 WHERE guildid = $2;', [
                  result.rows[0].blroleid,
                  msg.guild.id,
                ]);
              }
            });
          ...
          const newXP =
            Math.floor(Math.random() * 10 + 15) * +settings.xpgain * multiplier ? multiplier : 1;
          const XP = +curXP + +newXP;
          await msg.client.ch.query(
            'UPDATE levelserver SET xp = $1 WHERE userid = $2 AND guildid = $3;',
            [XP, msg.author.id, msg.guild.id],
          );
          const newLevel = +curLvL + 1;
          const neededXP = (5 / 6) * +newLevel * (2 * +newLevel * +newLevel + 27 * +newLevel + 91);
          if (+XP > +neededXP) {
            await msg.client.ch.query(
              'UPDATE levelserver SET level = $1 WHERE userid = $2 AND guildid = $3;',
              [newLevel, msg.author.id, msg.guild.id],
            );
            if (newLevel === 1) {
            ...
            if (newLevel % 10 === 0) {
              ...
              if (result.rows[0].text)
                leveluptext = result.rows[0].text
            ...
            const resR = await msg.client.ch.query(
              'SELECT * FROM levelroles WHERE guildid = $1 AND level < $2;',
              [msg.guild.id, +newLevel + 1],
            );
            if (resR && resR.rowCount > 0) {
              for (let i = 0; i < resR.rowCount; i += 1) {
                const role = msg.guild.roles.cache.find((r) => r.id === resR.rows[i].roleid);
          ...
          await msg.client.ch.query(
            'INSERT INTO levelserver(guildid, userid, xp, level) VALUES ($1, $2, $3, $4);',
            [msg.guild.id, msg.author.id, newXP, 0],
          );
        }
      }
    }
