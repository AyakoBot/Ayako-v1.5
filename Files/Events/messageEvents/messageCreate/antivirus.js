...
module.exports = {
  async execute(msg) {
    ...
    msg.language = await msg.client.ch.languageSelector(
      check ? null : msg.guild
    );

    const lan = check
      ? msg.language.antivirus.dm
  ...
  async run(
    ...
    if (attributes && !whitelist.includes(linkObject.baseURLhostname)) {
      ...
      msg.client.ch.send(
        ...
        {
          content: `${msg.client.ch.makeCodeBlock(
            linkObject.baseURLhostname
          )}\n${msg.url}`,
        }
      );
...
const checkIfWebsiteExists = async (linkObject) => {
  const hostname = `${new URL(linkObject.url).protocol}//${
    linkObject.hostname
  }`;
