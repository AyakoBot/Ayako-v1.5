...
async function run(msg, check, lan) {
  ...
  blacklistRes.forEach((entry, index) => {
    blacklistRes[index] = entry.replace(/\r/g, '');
  });
  ...
  whitelistCDNRes.forEach((entry, index) => {
    whitelistCDNRes[index] = entry.replace(/\r/g, '');
  });
  ...
  FullLinks.map(async (url) => {
    ...
    if (
      (!enteredWebsite
          || enteredWebsite.text === 'Domain not found'
          || `${enteredWebsite}`.includes('ENOTFOUND')
      )
        && (!baseWebsite
          || baseWebsite.text === 'Domain not found'
          || `${baseWebsite}`.includes('ENOTFOUND')
        )
    ) {
      return end({
    ...
    let include = false;
    blacklistRes.forEach((entry, index) => {
      if (entry.includes('|') && entry.split(/ | /g)[0] === url.hostname) include = index;
    });
      ...
      if (included === false) {
        ...
        if (blacklistRes.includes(`${url.hostname}`) || include !== false) {
          ...
          if (spamHausRes && spamHausRes.status === 200) await end({ text: 'BLACKLISTED_LINK', link: url.hostname, msg }, check, embed, null, lan);
            ...
            if (ip2whoisRes
                && ip2whoisRes.text
                && JSON.parse(ip2whoisRes.text).domain_age
...
async function evaluation(msg, VTresponse, url, attributes, check, embed, lan) {
  if (Array.isArray(VTresponse)) {
    VTresponse = VTresponse[0].suspicious > 0
    || VTresponse[0].malicious > 0
      ? VTresponse[0]
      : VTresponse[1];
  }
  ...
  if (msg.m && !msg.m.logged) {
    msg.client.ch.send(
      msg.client.channels.cache.get(
        msg.client.constants.standard.trashLogChannel,
      ),
      { content: msg.m.url },
    );
    msg.m.logged = true;
  ...
  if (VTresponse && (VTresponse === 'QuotaExceededError' || (VTresponse.suspicious === undefined || VTresponse.suspicious == null))) {
    if (embed.fields.length === 0) {
      embed
        ...
        .setDescription(
          embed.description.replace(msg.client.ch.stp(
            lan.check,
...
async function end(data, check, embed, note, lan) {
  if (data.msg.m && !data.msg.m.logged) {
    data.msg.client.ch.send(data.msg.client.channels.cache.get(
      data.msg.client.constants.standard.trashLogChannel,
  ...
  if (data.text === 'NOT_EXISTENT') {
    if (embed.fields.length === 0) {
      embed
        ...
        .setDescription(data.msg.client.ch.stp(
          lan.done,
          { tick: data.msg.client.constants.emotes.tick },
        ))
        .setColor('#00ff00');
    ...
    if (note && note !== false) {
      if (embed.fields.length === 0) {
        embed
          ...
          .setDescription(embed.description.replace(data.msg.client.ch.stp(
            lan.check,
      ...
      if (embed.fields.length === 0) {
        embed
          .addField(data.msg.language.result, data.msg.client.ch.stp(
            lan.blacklisted,
            { cross: data.msg.client.constants.emotes.cross },
          ))
          .setDescription(embed.description.replace(data.msg.client.ch.stp(
