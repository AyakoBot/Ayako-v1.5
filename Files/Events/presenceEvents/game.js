...
module.exports = {
  async execute(oldPresence, newPresence) {
    ...
    if (oldGames && newGames) {
      ...
      oldGames.forEach(async (game) => {
        game.when = 'old';
        oldPlayedGames.push(game);
      });
      newGames.forEach(async (game) => {
        game.when = 'new';
        newPlayedGames.push(game);
      });
      oldPlayedGames.forEach((game) => {
        newPlayedGames.filter((g) => game !== g);
      });
      newPlayedGames.forEach((game) => {
        oldPlayedGames.filter((g) => game !== g);
      });
      oldPlayedGames.forEach((game) => {
