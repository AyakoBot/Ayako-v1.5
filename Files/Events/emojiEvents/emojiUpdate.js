		...
		if (res && res.rowCount > 0) {
			...
			if (channels && channels.length > 0) {
				...
				const embed = new Discord.MessageEmbed()
					...
					.setColor(con.color)
					.setTimestamp();
				if (entry && entry.id) embed.setDescription(ch.stp(lan.description.withUser, {user: entry.executor, newEmoji: newEmoji, oldEmoji: oldEmoji}));
				else embed.setDescription(ch.stp(lan.description.withoutUser, {newEmoji: newEmoji, oldEmoji: oldEmoji}));
