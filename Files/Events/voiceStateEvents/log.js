        ...
        if (
          ...
          if (entry) {
            if (ch.getUnix(entry.id) > Date.now() - 3000)
              embed.addField(
                language.deaf,
                ch.stp(lan.deafAudit, {
                  oldState,
                  newState,
                  user: entry.executor,
                }),
              );
            else embed.addField(language.deaf, ch.stp(lan.deaf, { oldState, newState }));
          } else embed.addField(language.deaf, ch.stp(lan.deafNoAudit, { oldState, newState }));
        }
        if (
          ...
          embed.addField(language.mute, ch.stp(lan.mute, { oldState, newState }));
        } else if (
          oldState.selfMute === newState.selfMute &&
          oldState.serverMute !== newState.serverMute
        ) {
          ...
          if (entry) {
            if (ch.getUnix(entry.id) > Date.now() - 3000)
              embed.addField(
                language.mute,
                ch.stp(lan.muteAudit, {
                  oldState,
                  newState,
                  user: entry.executor,
                }),
              );
            else embed.addField(language.mute, ch.stp(lan.mute, { oldState, newState }));
          } else embed.addField(language.mute, ch.stp(lan.muteNoAudit, { oldState, newState }));
        }
        ...
        if (
          ...
          if (oldState.suppress !== newState.suppress) {
            ...
            if (oldState.suppress)
              embed.addField('\u200b', ch.stp(lan.setSuppress, { newChannel: newState.channel }));
            else if (newState.suppress)
              embed.addField('\u200b', ch.stp(lan.unsetSuppress, { newChannel: newState.channel }));
          }
