    ...
    if (oldState.channel !== newState.channel) {
      ...
      if (mute) {
        ...
        const res = await client.ch.query(
          'SELECT * FROM warns WHERE userid = $1 AND guildid = $2 AND closed = false;',
          [user.id, guild.id],
        );
        const language = await client.ch.languageSelector(guild.id);
        if (res && res.rowCount > 0 && newState.serverMute) {
          ...
          const res2 = await client.ch.query(
            'SELECT * FROM warns WHERE userid = $1 AND guildid = $2 AND closed = true;',
            [user.id, guild.id],
          );
          if (res2 && res2.rowCount > 0 && !newState.serverMute) {
            ...
            const res3 = await client.ch.query(
              'SELECT * FROM warns WHERE userid = $1 AND guildid = $2 AND closed IS NULL;',
              [user.id, guild.id],
            );
