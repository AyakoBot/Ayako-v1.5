	...
	async execute(msg) {
		...
		async function proceed(proceed, module) {
			if (proceed == false) {
				...
				if (modRoleRes) return msg.client.emit(`mod${msg.client.ch.CFL(module.name)}Add`, msg.author, user, reason, msg);
				else msg.delete().catch(() => {});
			} else return msg.client.emit(`mod${msg.client.ch.CFL(module.name)}Add`, msg.author, user, reason, msg);
		}
	}
};