...
module.exports = {
	...
	type: 1,
	async exe(msg) {
		const res = await msg.client.ch.query(`SELECT * FROM cooldowns WHERE guildid = '${msg.guild.id}';`);
		if (msg.args[1] && msg.args[1].toLowerCase() === msg.language.edit) this.edit(msg);
		else {
			if (res && res.rowCount > 0) {
				const channelText = [];
				for (const row of res.rows) {
					const channel = msg.client.channels.cache.get(row.channelid);
					if (channel) channelText.push(`${channel} / \`${channel.name}\` / \`${channel.id}\` - \`${row.command} - ${ms(parseInt(row.cooldown))}\``);
					else msg.client.ch.query(`DELETE FROM cooldowns WHERE channelid = '${row.channelid}';`);
				}
				const r = res.rows[0];

				const embed = new Discord.MessageEmbed()
					.setDescription(channelText.join())
					.addFields(
						{name: msg.lan.cooldowns, value: r.active ? msg.client.constants.emotes.tick+' '+msg.language.enabled : msg.client.constants.emotes.cross+' '+msg.language.disabled, inline: false},
					)
			...
			for (let i = 0; i < Object.keys(editAnswers).length; i++) {
				...
				if (editAnswers[name[i]].includes(answer)) {
					...
					if (msg.client.constants.commands.settings.edit.blacklist[name[i]] == 'number') {
						...
						if (r.words && words.length > 0) r.words.push(words);
						else r.words = words;
						const editedEmbed = new Discord.MessageEmbed()
							.setAuthor(msg.lan2.author, msg.client.constants.emotes.settingsLink, msg.client.constants.standard.invite)
							.setDescription(msg.client.ch.stp(msg.lan.done, {loading: msg.client.constants.emotes.loading})+'\n\n'+msg.client.ch.stp(msg.lan.edited, {edited: msg.client.ch.stp(msg.lan.edit[name[i]].name.replace(/\*/g, ''), {trigger: msg.lan.edit[name[i]].trigger.map(e => `${e}`), amount: '-'})}))
						...
						if (words.length > 0) msg.client.ch.query(`UPDATE blacklist SET ${name[i]} = ARRAY[${words}] WHERE guildid = '${msg.guild.id}';`);
						else msg.client.ch.query(`UPDATE blacklist SET ${name[i]} = null WHERE guildid = '${msg.guild.id}';`);
						const index = msg.args.indexOf(msg.language.edit);
						msg.args.splice(index, 1);
						msg.m = m;
