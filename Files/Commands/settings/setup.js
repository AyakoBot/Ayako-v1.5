  ...
  async execute(msg, answer) {
    ...
    async function yesFunc(message, clickButton) {
      for (
        ...
        const values = [];
        const cols = names;
        const vals = msg.client.constants.commands.settings.setupQueries[msg.file.name].vals[index];
        vals.forEach((val) => {
          if (typeof val === 'string') values.push(msg.client.ch.stp(val, { msg }));
        ...
        await msg.client.ch.query(
          `INSERT INTO ${
            msg.client.constants.commands.settings.tablenames[msg.file.name][index]
          } (${cols}) VALUES (${valDeclaration});`,
          values,
        );
      }
