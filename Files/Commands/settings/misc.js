...
module.exports = {
  ...
  async log(oldSettings, msg, newSettings) {
    ...
    if (type === 'edited') {
      ...
      for (let i = 0; i < Object.entries(lengthVar).length; i += 1) {
        if (Object.entries(oldSettings)[i][1] !== Object.entries(newSettings)[i][1])
          changed.push([
            [Object.entries(oldSettings)[i][0], Object.entries(oldSettings)[i][1]],
            [Object.entries(newSettings)[i][0], Object.entries(newSettings)[i][1]],
          ]);
      }
      embed
        ...
        .setDescription(
          !oldSettings.id
            ? msg.client.ch.stp(msg.language.selfLog.description, {
                msg,
                setting: msg.file.name,
              })
            : msg.client.ch.stp(msg.language.selfLog.descriptionwithID, {
                msg,
                setting: msg.file.name,
                id: newSettings.id,
              }),
      ...
      if (changed.length) {
        changed.forEach((change) => {
          if (
            Array.isArray(change[0][1]) &&
            Array.isArray(change[1][1]) &&
            change[0][1].equals(change[1][1])
          )
            return;
          embed.addFields({
            name: `${
              msg.lan[change[1][0] === 'active' ? 'type' : change[1][0]]?.includes('{{amount}}')
                ? msg.client.ch.stp(msg.lan[change[1][0] === 'active' ? 'type' : change[1][0]], {
                    amount: '--',
                  })
                : msg.lan[change[1][0] === 'active' ? 'type' : change[1][0]]
            }`,
            value:
              `__${msg.lanSettings.oldValue}__: ${
                Array.isArray(change[0][1])
                  ? change[0][1].map((c) =>
                      change[0][0]?.includes('role')
                        ? ` <@&${c}>`
                        : change[0][0]?.includes('channel')
                        ? ` <#${c}>`
                        : change[0][0]?.includes('user')
                        ? ` <@${c}>`
                        : ` ${c}`,
                    )
                  : change[0][1] == null
              ...
              `__${msg.lanSettings.newValue}__: ${
                Array.isArray(change[1][1])
                  ? change[1][1].map((c) =>
                      change[1][0]?.includes('role')
                        ? ` <@&${c}>`
                        : change[1][0]?.includes('channel')
                        ? ` <#${c}>`
                        : change[1][0]?.includes('user')
                        ? ` <@${c}>`
                        : ` ${c}`,
                    )
                  : change[1][1] == null
      ...
      for (let i = 0; i < Object.entries(newSettings).length; i += 1) {
        if (Object.entries(oldSettings)[i][1] !== Object.entries(newSettings)[i][1])
          changed.push([
            [Object.entries(oldSettings)[i][0], Object.entries(oldSettings)[i][1]],
            [Object.entries(newSettings)[i][0], Object.entries(newSettings)[i][1]],
