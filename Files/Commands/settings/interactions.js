...
module.exports = {
  ...
  async execute(msg) {
    const res = await msg.client.ch.query(
      `SELECT * FROM interactions WHERE guildid = '${msg.guild.id}';`,
    );
    if (
      msg.args[1] &&
      msg.args[1].toLowerCase() === msg.language.edit &&
      msg.member.permissions.has(new Discord.Permissions(this.perm))
    )
      this.edit(msg);
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addFields({
          ...
          value: `${
            r.size
              ? `${msg.client.constants.emotes.small} ${msg.lan.small}`
              : `${msg.client.constants.emotes.big} ${msg.lan.big}`
          }`,
          inline: false,
        })
      ...
      if (msg.member.permissions.has(new Discord.Permissions(this.perm)))
        embed.setDescription(
          `${msg.client.ch.stp(msg.lan.howToEdit, {
  ...
  async edit(msg) {
    ...
    if (res && res.rowCount > 0) {
      ...
      const embed = new Discord.MessageEmbed()
        .setDescription(
          `__${msg.lan2.howToEdit}__\n\n${msg.client.ch.stp(msg.lan2.block.name, {
            trigger: msg.lan2.block.trigger.includes('`')
              ? msg.lan2.block.trigger
              : msg.lan2.block.trigger.map((f) => `\`${f}\``),
          })}\n${
            r.block && r.block.length
              ? `${r.block.map((b) => ` \`${b}\``)}`.length
                ? `${r.block.map((b) => ` \`${b}\``)}`
        ...
        .addFields({
          name: msg.client.ch.stp(msg.lan2.size.name, {
            trigger: msg.lan2.size.trigger.includes('`')
              ? msg.lan2.size.trigger
              : msg.lan2.size.trigger.map((f) => `\`${f}\``),
          }),
          value: `${
            r.size
              ? `${msg.client.constants.emotes.small} ${msg.lan.small}`
      ...
      const collected = await msg.channel.awaitMessages({
        filter: (me) => me.author.id === msg.author.id,
        max: 1,
        time: 60000,
      });
      ...
      for (let i = 0; i < Object.keys(msg.lan.edit).length; i += 1) {
        ...
        if (name[i] !== 'author' && name[i] !== 'howToEdit') {
          ...
          for (let j = 0; j < Object.keys(editAnswers[name[i]]).length; j += 1) {
            editAnswers[name[i]][j] = editAnswers[name[i]][j].replace(/`/g, '');
          }
        }
      }
      for (let i = 0; i < Object.keys(editAnswers).length; i += 1) {
        ...
        if (editAnswers[name[i]].includes(answer)) {
          ...
          if (msg.lan2[name[i]].recommended)
            editEmbed.setDescription(
              `**${msg.client.ch.stp(msg.lan.edit[name[i]].name, {
                trigger: msg.lan.edit[name[i]].trigger.map((e) => `\`${e}\``),
                amount: '-',
            ...
            editEmbed.setDescription(
              `**${msg.client.ch.stp(msg.lan.edit[name[i]].name, {
                trigger: msg.lan2[name[i]].trigger.map((e) => `\`${e}\``),
                amount: '-',
              })}**`,
            );
          ...
          if (msg.client.constants.commands.settings.edit.interactions[name[i]] == 'boolean') {
            const boolAnswer =
              answer == msg.lan.big ? true : answer == msg.lan.small ? false : undefined;
            if (boolAnswer == undefined) this.notValid(msg, collected.first(), name[i]);
              ...
              const editedEmbed = new Discord.MessageEmbed()
                ...
                .setDescription(
                  ...
                  })}\n\n${msg.client.ch.stp(msg.lan.edited, {
                    edited: msg.client.ch.stp(msg.lan.edit[name[i]].name.replace(/\*/g, ''), {
                      trigger: msg.lan.edit[name[i]].trigger.map((e) => `${e}`),
                      amount: '-',
                    }),
                  })}`,
              ...
              msg.client.ch.query(
                `UPDATE interactions SET ${name[i]} = ${answer} WHERE guildid = '${msg.guild.id}';`,
              );
            ...
            msg.client.constants.commands.settings.edit.interactions[name[i]] == 'string'
          ) {
            const interactionFiles = fs
              .readdirSync('./Files/Commands/interactions')
            ...
            args.forEach((arg) => {
              const file =
                interactions.get(arg) ||
                interactions.find((cmd) => cmd.aliases && cmd.aliases.includes(answer));
              if (!file) return this.notValid(msg, m, name[i]);

              files.push(file);
              if (r.block && r.block.length && r.block.includes(arg))
                r.block.splice(`'${r.block.indexOf(arg)}'`, 1);
              else newWords.push(`'${arg}'`);
            });
            if (files.length < 1) {
              this.notValid(msg, m, name[i]);
              return;
            }
            if (r.block)
              for (let j = 0; j < r.block.length; j += 1) {
                if (!r.block[j].startsWith("'") && !r.block[j].endsWith("'"))
                  r.block[j] = `'${r.block[j]}'`;
              }
            ...
            if (r.block.length)
              msg.client.ch.query(
                `UPDATE interactions SET ${name[i]} = ARRAY[${r.block}] WHERE guildid = '${msg.guild.id}';`,
              );
            else
              msg.client.ch.query(
                `UPDATE interactions SET ${name[i]} = null WHERE guildid = '${msg.guild.id}';`,
              );
            collected
              .first()
              .delete()
            ...
            const endEmbed = new Discord.MessageEmbed()
              ...
              .setDescription(
                msg.client.ch.stp(msg.lan.done, { loading: msg.client.constants.emotes.loading }),
              )
              .addField(
                msg.lan.block,
                `${
                  r[name[i]] && r[name[i]] !== []
                    ? `${r[name[i]].map((word) => ` \`${word}\``)}`.length
                      ? `${r[name[i]].map((word) => `\`${word}\``)}`.replace(/\n/g, '')
                      : msg.language.none
                    : msg.language.none
                }`,
              );
  ...
  async setup(msg) {
    ...
    if (answer == msg.language.yes) {
      await msg.client.ch.query(
        `INSERT INTO interactions (guildid, size) VALUES ('${msg.guild.id}', true);`,
      );
      collected
        .first()
        .delete()
      ...
      const endEmbed = new Discord.MessageEmbed()
        ...
        .setDescription(
          msg.client.ch.stp(msg.lan.setup.done, { loading: msg.client.constants.emotes.loading }),
        );
