...
module.exports = {
	...
	displayEmbed(msg, r) {
		...
		if (r.stoprole) {
			if (sep.rawPosition > stop.rawPosition) for (let i = stop.rawPosition+1; i < msg.guild.roles.highest.rawPosition && i < sep.rawPosition; i++) affectedRoles.push(msg.guild.roles.cache.find(r => r.rawPosition == i));
			else for (let i = sep.rawPosition+1; i < msg.guild.roles.highest.rawPosition && i < stop.rawPosition; i++) affectedRoles.push(msg.guild.roles.cache.find(r => r.rawPosition == i));
		} else if (sep.rawPosition < msg.guild.roles.highest.rawPosition) for (let i = sep.rawPosition+1; i < msg.guild.roles.highest.rawPosition && i < msg.guild.roles.highest.rawPosition; i++) affectedRoles.push(msg.guild.roles.cache.find(r => r.rawPosition == i));
		const embed = new Discord.MessageEmbed();
		if (r.isvarying == true) {
			embed.addFields(
				{
