				...
				if (res !== undefined) {
					if (res.rowCount !== 0) {
						if (msg.guild.member(msg.author).roles.cache.has(role.id)) {
							msg.guild.member(msg.author).roles.remove(role).catch(() => {});
							const embed = new Discord.MessageEmbed()
								.setColor('b0ff00')
								.setDescription(`You no longer have the ${role} role`);
							msg.channel.send(embed).then((m) => {m.delete({ timeout: 10000 }).catch(() => {});}).catch(() => {});
						} else {
							msg.reply('You dont have this role, therefore i cant remove it').then((m) => {m.delete({ timeout: 10000 }).catch(() => {});}).catch(() => {});
							return;
						}
					} else {
