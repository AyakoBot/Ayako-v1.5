...
module.exports = {
	...
	async execute(msg) {
		...
		if (res.rows[0].willis == null) {
			embed.setDescription(msg.client.constants.emotes.tick+' You are NOT participating!\nGo to <#805839305377447936> and follow the instructions to enter')
				.setColor('YELLOW');
			msg.channel.send(embed);
		} else
		if (res.rows[0].willis.includes(id)) {
			embed.setDescription(msg.client.constants.emotes.tick+' You are participating! Good Luck!')
				.setColor('GREEN');
			msg.channel.send(embed);
		} else {
			embed.setDescription(msg.client.constants.emotes.tick+' You are NOT participating!\nGo to <#805839305377447936> and follow the instructions to enter')
