...
const getEndTime = (msg, startArg, now) => {
  ...
  const args = msg.args.slice(startArg).map((a) => {
    const validArg = a.replace(/\./, ',');
    return Math.abs(ms(validArg)) ? Math.abs(ms(validArg)) : a;
  });
  ...
  const endTime = Math.round(timeArgs.reduce((a, b) => a + b, now));
