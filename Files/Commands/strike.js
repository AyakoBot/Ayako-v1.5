module.exports = {
	...
	type: 'mod',
	async execute(msg) {
		const res = await msg.client.ch.query('SELECT * FROM autopunish WHERE guildid = $1 AND active = true;', [msg.guild.id]);
		...
		async function proceed(proceed, module) {
			if (proceed == false) {
				...
				if (modRoleRes) return msg.client.emit(`mod${msg.client.ch.CFL(module.name)}Handler`, msg.author, user, reason, msg);
				else msg.delete().catch(() => { });
			} else return msg.client.emit(`mod${msg.client.ch.CFL(module.name)}Handler`, msg.author, user, reason, msg);
		}
	}
};