...
module.exports = {
  ...
  async execute(msg) {
    ...
    args.map(async (rawArg) => {
      const arg = rawArg.toLowerCase();
      if (Number.isNaN(arg)) {
        if (arg.includes('<@') && arg.includes('>')) {
          const userID = arg.replace(/\D+/g, '');
          const user = await msg.client.users.fetch(userID).catch(() => {
            failed.push(arg);
          });
          if (user && user.id) {
            user.source = 'massban';
            user.guild = msg.guild;
            const bannedUser = bans.find((thisBans) => thisBans.user.id === user.id);
            if (bannedUser) {
        ...
        const user = await msg.client.users.fetch(arg).catch(() => {
          failed.push(arg);
        });
        if (user && user.id) {
          const bannedUser = bans.find((thisBans) => thisBans.user.id === arg);
          if (bannedUser) {
            failed.push(`${arg} ${msg.lan.already}`);
          } else if (user.id !== msg.author.id) {
            const member = await msg.guild.members.fetch(user.id).catch(() => {});
    ...
    if (uniqueUsers.length !== 0) {
      const replyEmbed = new Discord.MessageEmbed()
        ...
        .setAuthor(
          msg.lan.descSLoading,
          msg.client.constants.loadingLink,
    ...
    if (uniqueFails.length !== 0) {
      const replyEmbed = new Discord.MessageEmbed()
        ...
        .setAuthor(
          msg.lan.descFLoading,
          msg.client.constants.crossLink,
          msg.client.constants.standard.invite,
        )
        .setTimestamp();
      const m = await msg.client.ch.reply(msg, replyEmbed);
    ...
    const interval = setInterval(async () => {
      if (uniqueUsers.length === descS.length && uniqueFails.length === descF.length) {
        const logembed = new Discord.MessageEmbed()
          .setAuthor(
            msg.client.ch.stp(msg.lan.log.author, { user: msg.author, amount: uniqueUsers.length }),
          )
